...
function openModalInit() {
    let btnActiveModal = document.querySelectorAll('.btn_open_modal');
    eventModal(btnActiveModal);
}

// Close modal
function closeModalInit() {
    let btnCloseModal = document.querySelectorAll('.btn_close_modal');
...
function eventModal(elementActionModal) {
    elementActionModal.forEach(element => {
        element.addEventListener('click', () => {
            let elementId = element.getAttribute('data-modal-id');
...
$(function() {
    ...
    function getPraticienName(data) {
        ...
        $.ajax({
            ...
            data: {
                praticien_name: data,
                //praticien_type: $('#praticien-name').val()
            },
    ...
    let btnNewMed = $(".btn_new_med");
    let nameOfMed = $("#med-option-name");
    let quantityOfMed = $("#med-quantity");
    quantityOfMed.val(0);
    let medList = $(".med_list");

    btnNewMed.click(function() {

        if (nameOfMed.val() != "defaultValue" && quantityOfMed.val() > 0) {

            let medItem = $(".med_item");

            // ADD IN HTML MED-NAME AND MED-QUANTITY
            if (medItem) {

                let medIsAlreadyAddInList = false;
                ...
                if (medIsAlreadyAddInList == false) {
                    medList.append(
                        '<tr class="med_item"><td class="med_name"><input type="text" name="med_depotlegal_' + (medItem.length + 1) + '" value="' + nameOfMed.children(":selected").text() + '"></td><td class="med_quantity"><input type="number" name="med_quantity_' + (medItem.length + 1) + '" value="' + quantityOfMed.val() + '" min="0"></td><td class="btn_delete_med_item"><i class="bx bx-trash"></i></td></tr>'
                    );
                } else {
    ...
    function removeMedItem() {
        let btnDeleteMedItem = $(".btn_delete_med_item");
